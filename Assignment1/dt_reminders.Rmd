...
To compare two numbers we can use:
...
## To find the row numbers corresponding to a particular value

```{r}
which(mpg$model == "civic")
```
...
#The following code reorders the levels so "5" is first, then "8" is second
#and the remaining levels are then unchanged in order
mpg1$cyl<-fct_relevel(mpg1$cyl,"5","8")
```
...
## Finding strings containging a substring

* This will find a smaller string inside a bigger string. It returns a Boolean list of where the substring was found.

```{r}
str_detect(mpg$trans, "5\\)")
```
...
* Calculate the absolute value of a list
...
* Add up all the values in a numeric list
```{r}
sum(mpg$cyl)
```
